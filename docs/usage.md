...
| `plainship publish`         | 发布到服务器(只发布已提交源码构建出的内容,发布前四重校验)                          |
| `plainship connect <地址>`  | 配置并验证服务器连接(url 写入 `plainship.yaml`,令牌写入 `.plainship/server.token`) |
| `plainship status [路径]`   | 查看 Git / 变更 / 构建 / 发布状态                                                  |
| `plainship dev [--addr]`    | 本地开发模式:监听变更自动重建,SSE 热更新浏览器(默认 `:8080`)                       |
| `plainship preview`         | 发布前在本地预览已构建的站点                                                       |
| `plainship config`          | 配置 CLI 行为(lang 等)                                                             |
...
| 命令                       | 说明                                                                      |
| -------------------------- | ------------------------------------------------------------------------- |
| `plainship-server serve`   | 启动服务器(`--addr` `--data` `--token`;不带 `--token` 时自动生成并持久化) |
...
server:
  ...
  # 令牌不写入此文件(避免进 Git 历史),用 plainship connect 写入 .plainship/server.token
...
GFM(GitHub Flavored Markdown):标题,段落,粗体,斜体,链接(自动解析为根相对 URL),图片,列表,引用,代码块(语法高亮),表格,任务列表.正文中的原始 HTML 默认被剔除(防发布站点 XSS);需要直通时设置 `markdown.unsafe: true`.
